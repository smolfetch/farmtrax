...
namespace farmtrax {
    ...
    public:
        ...
        explicit ObstacleAvoider(const std::vector<concord::Polygon> &obstacles, const concord::Datum &datum = concord::Datum{}) 
            : datum_(datum) {
    ...
    private:
        ...
        BPolygon inflate_polygon(const BPolygon &polygon, float distance) const {
            ...
            try {
                boost::geometry::buffer(polygon, buffered, dist_strategy, side_strategy, 
        ...
        std::vector<std::shared_ptr<Swath>> process_swath(const std::shared_ptr<const Swath> &swath) {
            ...
            for (size_t i = 0; i < cut_segments.size(); ++i) {
                ...
                auto cut_swath = std::make_shared<Swath>(
                    create_swath(start_point, end_point, swath->type, "")
                );
        ...
        void sort_segments_by_position(std::vector<BLineString> &segments, 
            ...
            std::sort(segments.begin(), segments.end(), 
                [&start_point](const BLineString &a, const BLineString &b) {
