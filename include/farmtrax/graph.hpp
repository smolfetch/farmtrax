    ...
    private:
        ...
        void add_nearest_connections(Vertex vertex, const farmtrax::BPoint& point, size_t current_line_id) {
            ...
            for (const auto& near : nearest) {
                ...
                if (distance > 150.0) continue; // Adjustable threshold
        ...
        double calculate_connection_score(const farmtrax::BPoint& from_point,
                                         const farmtrax::BPoint& to_point,
                                         size_t from_line_id,
                                         size_t to_line_id,
        ...
        double calculate_clustering_bonus(const farmtrax::BPoint& from_point, 
                                         const farmtrax::BPoint& to_point) const {
        ...
        std::pair<size_t, bool> find_best_local_candidate(const farmtrax::BPoint& current_position,
            ...
            for (const auto& candidate : candidates) {
                ...
                if (visited_lines[line_id]) continue; // Skip visited lines
        ...
        double calculate_field_pattern_score(const farmtrax::BPoint& current_pos,
                                            const farmtrax::BPoint& candidate_pos,
                                            size_t candidate_line_id,
                                            double distance) const {
        ...
        double calculate_direction_penalty(const farmtrax::BPoint& current_pos,
                                          const farmtrax::BPoint& candidate_pos,
            ...
            if (move_length < 1e-6 || line_length < 1e-6) return 0.0;
        ...
        double calculate_pattern_bonus(const farmtrax::BPoint& current_pos, size_t candidate_line_id) const {
            ...
            for (const auto& point : nearby_points) {
                ...
                if (line_id == candidate_line_id) continue;
